...
public partial class ProfileEditorViewModel : ObservableObject
{
    ...
    private void RecomputeMap()
    {
        ...
        // Přiblížení na jeden monitor: rozsah mapy se počítá jen z toho monitoru, ostatní se pak vykreslí
        // mimo/na okraji plátna (ořízne je ClipToBounds na obalujícím Borderu) - typicky uživatel klikal
        // jen na jednom monitoru, tak je škoda mu ho zobrazovat jako malý proužek uprostřed celé virtuální plochy.
        IReadOnlyList<MonitorBounds> boundsSource = FocusedMonitorIndex is int idx
            ? new[] { snapshot.Monitors[idx] }
            : snapshot.Monitors;

        int minX = boundsSource.Min(m => m.X);
        int minY = boundsSource.Min(m => m.Y);
        int maxX = boundsSource.Max(m => m.X + m.Width);
        int maxY = boundsSource.Max(m => m.Y + m.Height);
        ...
        for (int i = 0; i < snapshot.Monitors.Count; i++)
        {
            ...
            MapMonitorRects.Add(new MapMonitorRectViewModel
            {
                ...
                IsFocused = FocusedMonitorIndex == i
            });
        }
